...
rows=[]; report=[]
for y,d in DRAFT.items():
    dd=d.replace("-",""); start=(int(dd[:4]),dd); 
    import datetime as dt; D=dt.datetime.strptime(d,"%Y-%m-%d"); frm=(D-dt.timedelta(days=21)).strftime("%Y%m%d"); to=(D-dt.timedelta(days=1)).strftime("%Y%m%d")+"235959"
    for site,pat in SITES.items():
        u=pat.format(y=y); cdx=fetch(f"https://web.archive.org/cdx/search/cdx?url={urllib.parse.quote(u)}&from={frm}&to={to}&output=json&filter=statuscode:200&fl=timestamp&limit=-1",f"mock_raw/consensus2/cdx_{site}_{y}.json")
        try: snaps=[x[0] for x in json.loads(cdx)[1:]]
        except Exception: snaps=[]
        if not snaps: report.append((y,site,"no snapshot")); continue
        ts=snaps[-1]; html=fetch(f"https://web.archive.org/web/{ts}id_/https://{u}",f"mock_raw/consensus2/snap_{site}_{y}_{ts}.html",60)
...
w=csv.DictWriter(open("mock_consensus2_ranks.csv","w"),fieldnames=["pid","draft_year","site","rank","ts"]); w.writeheader(); [w.writerow(r) for r in rows]
