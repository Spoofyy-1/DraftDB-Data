"""Registered incumbent reproduction, development only, no test access.
...
def main():
    ...
    variants=[dict(id='incumbent_calendar_repaired',noscout=0,drafted_only=False,context=None),
              dict(id='incumbent_without_scout',noscout=1,drafted_only=False,context=None),
              dict(id='incumbent_without_scout_drafted',noscout=1,drafted_only=True,context=None),
              dict(id='incumbent_without_scout_team',noscout=1,drafted_only=False,context='ctx_team_'),
              dict(id='incumbent_without_scout_skill',noscout=1,drafted_only=False,context='ctx_skill_'),
              dict(id='incumbent_without_scout_all_context',noscout=1,drafted_only=False,context='ctx_')]
    weights={'rich':(0.,.75,.25),'thin':(.5,0.,.5)}
    plan=dict(variants=variants,folds=[2012,2013,2014],weights=weights,
              purpose='Incumbent model-only reproduction and feature diagnostics; not a certified replacement',
    ...
    state=dict(status='running',phase='incumbent reproduction · development only',started=time.time(),completed=0,total=len(variants),
    ...
    for v in variants:
        ...
        try:
            gg={**g,'noscout':v['noscout']}
            features=m['legacy_features']+[c for c in m['context_features'] if v['context'] and c.startswith(v['context'])]
        ...
        write(state)
    state['status']='completed';state['phase']='incumbent reproduction complete'
