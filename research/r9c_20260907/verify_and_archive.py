...
EXPECTED_FROZEN='39f3655d64e98fd0596f0420d0113d8bac12ecce353bc5e79b6075a637279fbf'
...
def main():
    ...
    frozen=json.loads(frozen_raw);assert len(frozen['files'])==58
    ...
    assert len(expected)==len(set(expected))==459 and len(plan['features'])==35 and len(plan['groups'])==38
    ...
    completion=json.loads((OUT/'completion.json').read_text());assert completion['status']=='completed' and completion['tasks']==459 and completion['no_model_promotion']
    state=json.loads((OUT/'state.json').read_text());assert state['status']=='completed' and state['completed']==state['total']==459 and state['active_workers']==0 and not state.get('error')
    ...
    assert summary['interpretation_allowed'] and summary['reference_replays_passed']==3 and not summary['reference_replays_pending'] and not summary['pending'] and len(summary['feature_groups'])==38
    candidates={e['task_id']:e for e in state['candidates']};assert len(candidates)==459 and set(candidates)==set(expected)
    ...
    assert json.loads((ROOT/'data/source_verification.json').read_text())['model_eligible']is False and plan['source_admission']['source_model_eligible_flag_remains_false']
    print('Verified 459 records, 3 exact references, 58 frozen files and exact complete summary.',flush=True)
    ...
    proof={'passed':True,'models_fitted':0,'records':459,'reference_replays_exact':3,'frozen_files_unchanged':58,'frozen_manifest_sha256':EXPECTED_FROZEN,'preregistration_exact':True,'queue_registration_exact':True,'append_log_all_hashes_exact':True,'complete_summary_recomputed_exact':True,'compact_candidates_exact':True,'single_feature_groups':35,'whole_family_groups':3,'approved_source_projection_sha256':plan['source_admission']['source_candidate_sha256'],'source_global_model_eligible_remains_false':True,'all_pre2019_development_only':True,'no_model_promotion':True,'task_archive_manifest':file_info(OUT/'task_archive_manifest.json'),'chunks':len(chunks),'original_bytes':archive['original_bytes'],'compressed_bytes':archive['compressed_bytes'],'max_chunk_bytes':max(c['bytes'] for c in chunks),'all_record_roundtrips_byte_exact':True,'elapsed_seconds':completion['elapsed_seconds'],'trials_per_minute':459*60/completion['elapsed_seconds'],'verification_seconds':time.perf_counter()-start,'verification_code_sha256':sha(Path(__file__).read_bytes()),'summary':file_info(OUT/'matched_summary.json')}
    write_json(OUT/'run_verification.json',proof)
    report=f"R9c verification passed: 459/459 immutable task records, all 3 full reference replays, all 58 frozen file hashes and the complete recomputed summary match. No model or held-out evaluation was run during verification.\n\nThe queue completed in {completion['elapsed_seconds']:.3f} seconds ({proof['trials_per_minute']:.2f} tasks/minute), with no errors. It covers 35 single new-data columns and three whole-family matched-control groups and remains a pre-2019 development diagnostic, with no model promotion. The added-width, correlated-permutation and retrospective source limitations in the registration still apply.\n\nAll original task files remain on the server. {len(chunks)} deterministic gzip chunks preserve their exact bytes ({archive['original_bytes']:,} original bytes; {archive['compressed_bytes']:,} compressed bytes); every record passed round-trip comparison. Each chunk is below 40 MB. task_archive_manifest.json records original file, task ID, byte length, SHA-256, chunk and line, plus compressed and uncompressed chunk hashes. To restore, decompress a chunk and remove exactly one final LF from each record line.\n\nThe source package remains model_eligible=False under its separate bounded diagnostic admission. Eight GP/shooting representations are flagged; no source certification or held-out claim follows this study. Full numerical diagnostics are in matched_summary.json. Verification deliberately performs no selection for the next study.\n"
