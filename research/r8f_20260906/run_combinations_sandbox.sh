#!/usr/bin/env bash
set -euo pipefail
root=/home/ubuntu/nba/handoff/r8f
...
exec sudo -n bwrap "${args[@]}" /usr/bin/setpriv --reuid="$(id -u)" --regid="$(id -g)" --clear-groups --no-new-privs --bounding-set=-all /opt/venv/bin/python -u /workspace/combinations.py "$@"
