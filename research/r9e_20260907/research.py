...
def prepare():
    ...
    resume=json.loads((OUT/'resume_input_verification.json').read_text());assert resume['count']==16
    for tid,info in resume['immutable_records_retained'].items():
        raw=(OUT/'tasks'/(tid+'.json')).read_bytes();assert hashlib.sha256(raw).hexdigest()==info['sha256'] and len(raw)==info['bytes']
        entry=json.loads(raw);X.validate_entry(entry,p);assert entry['config']['id']in p['reference_ids']
    for v in p['variants']:
        for seed in p['seeds']:
