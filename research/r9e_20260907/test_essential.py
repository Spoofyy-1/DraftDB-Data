import copy,json,itertools,hashlib,re,tempfile
...
assert all(re.fullmatch(r'[A-Za-z0-9_]+',f"{v['id']}_seed{seed}") for v in p['variants'] for seed in p['seeds'])
oldp=json.loads((Path(__file__).parent/'attempt1_registration/plan.json').read_text());oldc=json.loads((Path(__file__).parent/'attempt1_registration/results/capacity_check.json').read_text())
for old in oldp['requested_grid']:
 new_id=old['id'].replace('.','p');assert {k:v for k,v in old.items()if k!='id'}=={k:v for k,v in p['settings_by_id'][new_id].items()if k!='id'}
 assert oldc['designs'][old['id']]==cap['designs'][new_id]
 if old['id']in oldc['equality_signatures']:assert oldc['equality_signatures'][old['id']]==cap['equality_signatures'][new_id]
...
with tempfile.TemporaryDirectory(dir=Path(__file__).parent/'results') as tmp:
 rejects(lambda:Q.save_immutable(Path(tmp),{'task_id':'none_o0.5_n16_seed0'}))
 path,digest=Q.save_immutable(Path(tmp),{'task_id':'none_o0p5_n16_seed0'});assert path.is_file() and hashlib.sha256(path.read_bytes()).hexdigest()==digest
