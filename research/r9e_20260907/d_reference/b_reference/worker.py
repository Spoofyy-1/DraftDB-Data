"""Bounded five-input deletion and joint-vector controls; unchanged A/X model."""
import os
...
spec=importlib.util.spec_from_file_location('r9b_frozen_a',R/'a_reference/worker.py');A=importlib.util.module_from_spec(spec);spec.loader.exec_module(A);X=A.X;B=A.B
...
def load_plan():
    ...
    if _PLAN is None:
        ...
        assert p['diagnostic_only'] and p['no_confirmation_or_test_scoring'] and len(p['variants'])*len(p['seeds'])==330
        xp=X.load_plan()
...
def references():
    global _REF
    if _REF is None:_REF={(e['config']['id'],e['seed']):e for e in json.loads((R/'references.json').read_text())['records']}
    return _REF
...
def full_design(fold):
    ...
    if year not in _FULL:
        ...
        old=next(r for r in references()[(load_plan()['reference_id'],0)]['rows'] if r['season']==year)
...
def apply_variant(original,metadata,v,role):
    ...
    if v['kind']=='delete':
        assert len(affected)in [3,4,5] and set(affected)<=set(p['selected_columns']);out=original.drop(columns=affected).copy();invariants=None
    else:
        assert v['kind']=='permutation' and len(affected)in [2,3,4,5] and set(affected)<=set(p['selected_columns']) and v['permutation_seed']in p['permutation_seeds']
        out=original.copy();before=original[affected]
        values=B._vector_shuffle(before,metadata,'fifty:joint_input_ablation:'+':'.join(affected),v['permutation_seed'],role)
...
def run_variant(vid,seed):
    ...
    if vid in p['reference_ids']:return A.run_variant(vid,seed)
...
def validate_entry(entry,p=None):
    ...
    if v['id']in p['reference_ids']:X.exact_reference(entry,references()[(v['id'],seed)]);return
    ref=references()[(p['reference_id'],seed)]
...
def summarize_matched(entries):
    ...
    for entry in entries:
        ...
        if entry['config']['id']not in p['reference_ids']:
    ...
    common={'completed_tasks':len(tasks),'expected_tasks':330,'reference_replays_passed':48-len(missing),'reference_replays_pending':missing,'interpretation_allowed':not missing,'diagnostic_only':True,'automatic_promotion':False,'metric':'Pre2019 mean fold Spearman, not classification accuracy'}
    ...
    deletions=[];perms=[]
    for v in p['variants']:
        if v['id']in p['reference_ids']:continue
        result=paired(v)
        if result is not None:
            if v['kind']=='delete':deletions.append(result)
            else:result['permutation_seed']=v['permutation_seed'];perms.append(result)
    controls=[]
    for cols in p['subsets']:
        group=[r for r in perms if r['columns']==cols]
        if len(group)==3:
            rows=[r for item in group for r in item['paired_rows']]
            controls.append({'columns':cols,'source_fields':[p['column_meanings'][c] for c in cols],'permutation_mean':float(np.mean([r['score'] for r in rows])),'permutation_minus_full':float(np.mean([r['delta'] for r in rows])),'full_minus_permutation':-float(np.mean([r['delta'] for r in rows])),
    ...
    return {**common,'full_score':baseline,'ablations':deletions,'permutation_arms':perms,'permutation_controls':controls,'completed_deletions':len(deletions),'expected_deletions':16,'pending_tasks':330-len(tasks),'limitations':p['controls']}
