#!/usr/bin/env bash
set -euo pipefail
root=/home/ubuntu/nba/handoff/r9e
args=(--unshare-net --unshare-pid --unshare-ipc --unshare-uts --die-with-parent
      ...
      --chdir /workspace)
exec sudo -n bwrap "${args[@]}" /usr/bin/setpriv --reuid="$(id -u)" --regid="$(id -g)" --clear-groups --no-new-privs --bounding-set=-all /opt/venv/bin/python -u /workspace/saved_prediction_blends.py "$@"
