...
def setup(feature_columns):
    ...
    ns.update(FEATS=feature_columns,
              ...
              EL_COLS=[],WK_COLS=[],XDEV="cpu",_TFM=[None],
              XCFG=dict(objective="reg:squarederror",max_depth=3,learning_rate=.02,n_estimators=800,subsample=.7,device="cpu",tree_method="hist",colsample_bytree=.5,min_child_weight=8,reg_lambda=5.,reg_alpha=1.,n_jobs=3))
    exec(H_SOURCE,ns)
...
def cfg_of(m,g,cols=None):
    ...
    if m=="tabicl":
        kw={"random_state":int(os.environ.get("SEED_SHIFT","0")),"batch_size":32}
...
def member_preds(g,Btr,Bte,cols,trdf,k):
    ...
    def icl_pred(yl):
        key=np.asarray(yl,dtype=np.float64).tobytes()
        if key not in full_prediction_cache:
            full_prediction_cache[key]=H.predict("tabicl",Btr[cc].iloc[CI],np.asarray(yl)[CI],Bte[cc],cfg_of("tabicl",g,cols))
        return full_prediction_cache[key].copy()
