...
EXPECTED_FROZEN='18aeeca6b66ee022c7b588198afa0be3b4ca536e67a28633e74461aa0b37f5b9'
...
def main():
    ...
    frozen=json.loads(frozen_raw);assert len(frozen['files'])==48
    ...
    assert len(expected)==len(set(expected))==330 and len(plan['columns'])==47
    ...
    completion=json.loads((OUT/'completion.json').read_text());assert completion['status']=='completed' and completion['tasks']==330 and completion['no_model_promotion']
    state=json.loads((OUT/'state.json').read_text());assert state['status']=='completed' and state['completed']==state['total']==330 and state['active_workers']==0 and not state.get('error')
    ...
    records=[];record_info=[];refs=X.references();ref_count=0
    for tid in expected:
    ...
    assert summary['interpretation_allowed'] and summary['reference_replays_passed']==48 and not summary['reference_replays_pending'] and summary['pending_tasks']==0 and len(summary['ablations'])==16 and len(summary['permutation_arms'])==78 and len(summary['permutation_controls'])==26
    candidates={e['task_id']:e for e in state['candidates']};assert len(candidates)==330 and set(candidates)==set(expected)
    ...
    print('Verified 330 records, 48 exact references, 48 frozen files and exact complete summary.',flush=True)
    ...
    proof={'passed':True,'models_fitted':0,'records':330,'reference_replays_exact':48,'frozen_files_unchanged':48,'frozen_manifest_sha256':EXPECTED_FROZEN,'preregistration_exact':True,'queue_registration_exact':True,'append_log_all_hashes_exact':True,'complete_summary_recomputed_exact':True,'compact_candidates_exact':True,'new_higher_order_deletions':16,'joint_permutation_subsets':26,'all_pre2019_development_only':True,'no_model_promotion':True,'task_archive_manifest':file_info(OUT/'task_archive_manifest.json'),'chunks':len(chunks),'original_bytes':archive['original_bytes'],'compressed_bytes':archive['compressed_bytes'],'max_chunk_bytes':max(c['bytes'] for c in chunks),'all_record_roundtrips_byte_exact':True,'elapsed_seconds':completion['elapsed_seconds'],'trials_per_minute':330*60/completion['elapsed_seconds'],'verification_seconds':time.perf_counter()-start,'verification_code_sha256':sha(Path(__file__).read_bytes()),'summary':file_info(OUT/'matched_summary.json')}
    write_json(OUT/'run_verification.json',proof)
    report=f"R9b verification passed: 330/330 immutable task records, all 48 full reference replays, all 48 frozen file hashes and the complete recomputed summary match. No model or held-out evaluation was run during verification.\n\nThe queue completed in {completion['elapsed_seconds']:.3f} seconds ({proof['trials_per_minute']:.2f} tasks/minute), with no errors. It covers all 16 higher-order removals and all 26 joint-vector control subsets, with 48 exact prior-reference replays and remains a pre-2019 development diagnostic, with no model promotion. The deletion-width, correlated-permutation and retrospective source limitations in the registration still apply.\n\nAll original task files remain on the server. {len(chunks)} deterministic gzip chunks preserve their exact bytes ({archive['original_bytes']:,} original bytes; {archive['compressed_bytes']:,} compressed bytes); every record passed round-trip comparison. Each chunk is below 40 MB. task_archive_manifest.json records original file, task ID, byte length, SHA-256, chunk and line, plus compressed and uncompressed chunk hashes. To restore, decompress a chunk and remove exactly one final LF from each record line.\n\nFull numerical diagnostics are in matched_summary.json. Verification deliberately performs no selection for the next study.\n"
