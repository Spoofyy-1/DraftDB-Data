#!/usr/bin/env bash
set -euo pipefail
root=/home/ubuntu/nba/handoff/r9test
args=(--unshare-net --unshare-pid --unshare-ipc --unshare-uts --die-with-parent
      ...
      --ro-bind "$root/input_package" /input_package
      --ro-bind /home/ubuntu/nba/handoff/r9test_label_exports /label_exports
      --ro-bind /home/ubuntu/nba/handoff/r9h /h_reference
      --ro-bind /home/ubuntu/nba/handoff/r9g /reference
      --bind "$root/bundles" /prepared
      ...
      --chdir /code)
exec sudo -n bwrap "${args[@]}" /usr/bin/setpriv --reuid="$(id -u)" --regid="$(id -g)" --clear-groups --no-new-privs --bounding-set=-all /opt/venv/bin/python -u /code/prepare_bundles.py "$@"
