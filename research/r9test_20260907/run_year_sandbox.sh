#!/usr/bin/env bash
set -euo pipefail
year=${1:?one prediction year is required}
case "$year" in 2019|2020|2021|2022|2023|2024|2025) ;; *) exit 2;; esac
stage=/home/ubuntu/nba/handoff/r9test
bundle="$stage/bundles/$year"
prediction_dir="$stage/results/$year"
[[ -f "$stage/code/approval.json" ]] || exit 3
...
args=(--unshare-net --unshare-pid --unshare-ipc --unshare-uts --die-with-parent
      ...
      --ro-bind "$stage/code" /code --ro-bind "$bundle" /bundle
...
exec sudo -n bwrap "${args[@]}" /usr/bin/setpriv --reuid="$(id -u)" --regid="$(id -g)" --clear-groups --no-new-privs --bounding-set=-all /opt/venv/bin/python -u /code/predict_year.py
