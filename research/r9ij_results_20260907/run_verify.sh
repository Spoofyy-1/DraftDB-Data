...
out="$base/r9ij_verification/$study"
mkdir -p "$out"
...
--ro-bind /home/ubuntu/nba/.venv /opt/venv --ro-bind "$base/$study" /workspace --ro-bind "$base/r9g" /reference --ro-bind "$base/r9h" /h_reference
--ro-bind "$base/r9ij_verification/code" /auditcode --bind "$out" /verification
--ro-bind /home/ubuntu/.cache/huggingface/hub/models--jingang--TabICL /models/hub/models--jingang--TabICL
--setenv HF_HOME /models --setenv HF_HUB_OFFLINE 1 --setenv XDG_CACHE_HOME /tmp/cache --setenv OMP_NUM_THREADS 2 --setenv OPENBLAS_NUM_THREADS 2 --setenv R8_SANDBOX 1 --chdir /workspace)
exec sudo -n bwrap "${args[@]}" /usr/bin/setpriv --reuid="$(id -u)" --regid="$(id -g)" --clear-groups --no-new-privs --bounding-set=-all /opt/venv/bin/python -u /auditcode/verify_saved.py "$study"
