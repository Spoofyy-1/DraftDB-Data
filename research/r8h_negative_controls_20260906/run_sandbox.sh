#!/usr/bin/env bash
set -euo pipefail
root=/home/ubuntu/nba/handoff/r8h
args=(--unshare-net --unshare-pid --unshare-ipc --unshare-uts --die-with-parent
      ...
      --setenv R8_WORKERS "${R8_WORKERS:-2}" --setenv R8_SANDBOX 1 --setenv OMP_NUM_THREADS 3 --setenv OPENBLAS_NUM_THREADS 3
      --chdir /workspace)
for dev in /dev/nvidia0 /dev/nvidiactl /dev/nvidia-uvm /dev/nvidia-uvm-tools; do
  if [[ -e "$dev" ]]; then args+=(--dev-bind "$dev" "$dev"); fi
done
exec sudo -n bwrap "${args[@]}" /usr/bin/setpriv --reuid="$(id -u)" --regid="$(id -g)" --clear-groups --no-new-privs --bounding-set=-all /opt/venv/bin/python -u /workspace/research.py "$@"
