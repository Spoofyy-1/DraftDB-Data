...
def setup(feature_columns):
    ...
    ns.update(FEATS=feature_columns,
              ...
              XCFG=dict(objective="reg:squarederror",max_depth=3,learning_rate=.02,n_estimators=800,subsample=.7,device="cuda",tree_method="hist",colsample_bytree=.5,min_child_weight=8,reg_lambda=5.,reg_alpha=1.,n_jobs=6))
    exec(H_SOURCE,ns)
...
def member_preds(g,Btr,Bte,cols,trdf,k):
    ...
    if g.get("icl_topk",0):
        import xgboost as xgb
        imp=xgb.XGBRegressor(max_depth=3,n_estimators=300,learning_rate=0.05,subsample=0.8,colsample_bytree=0.6,n_jobs=8,device=H.XDEV,tree_method="hist",random_state=11).fit(Btr[cols],label(trdf,specs(g)[0],k)).feature_importances_
        ...
        for spec in specs(g):
            ...
            for w,yl in labels_for(spec):
                ...
                if g["hurdle"]:
                    ...
                    p=xgb.XGBClassifier(max_depth=3,learning_rate=0.03,n_estimators=500,subsample=0.7,colsample_bytree=0.5,min_child_weight=8,reg_lambda=5.0,n_jobs=8,device=H.XDEV,tree_method="hist",random_state=11).fit(Btr[cols],played).predict_proba(Bte[cols])[:,1]*pd.Series(p).rank(pct=True).values
    ...
    if float(g.get("star",0))>0:
        ...
        out["star"]=np.mean([xgb.XGBClassifier(max_depth=3,learning_rate=0.03,n_estimators=400,subsample=0.7,colsample_bytree=0.5,min_child_weight=5,reg_lambda=5.0,scale_pos_weight=3.0,n_jobs=8,device=H.XDEV,tree_method="hist",random_state=s).fit(Btr[cols],top).predict_proba(Bte[cols])[:,1] for s in (11,12,13)],0)
    return out
