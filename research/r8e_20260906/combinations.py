...
def main():
    ...
    state.update(status='running',phase='R8e · complementary feature combinations',total=len(plan['variants'])*len(seeds)+len(combos)*len(seeds),message=f'{len(winners)} statistics passed the preregistered development gate; testing {len(combos)} combinations.')
    write(state)
    ...
    state.update(status='completed',phase='R8e · screens and first combinations complete',current='Ready for review',message=f'Completed individual comparisons and gated combinations. {len(winners)} provisional development winners; no test selection.')
    write(state)
