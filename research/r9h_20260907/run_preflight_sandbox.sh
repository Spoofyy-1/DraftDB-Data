#!/usr/bin/env bash
set -euo pipefail
root=/home/ubuntu/nba/handoff/r9h
args=(--unshare-net --unshare-pid --unshare-ipc --unshare-uts --die-with-parent
      ...
      --bind "$root/data" /workspace/data
      --bind "$root/plan.json" /workspace/plan.json
      --bind "$root/source_manifest.json" /workspace/source_manifest.json
      ...
      --chdir /workspace)
exec sudo -n bwrap "${args[@]}" /usr/bin/setpriv --reuid="$(id -u)" --regid="$(id -g)" --clear-groups --no-new-privs --bounding-set=-all /opt/venv/bin/python -u /workspace/preflight.py "$@"
