...
def reference_records():
    global _REF_CACHE
    if _REF_CACHE is None:_REF_CACHE={(r['config']['id'],r['seed']):r for r in json.loads((ROOT/'w_reference.json').read_text())['records']}
    return _REF_CACHE

def frozen_x():
    global _FROZEN_X
    if _FROZEN_X is None:
        spec=importlib.util.spec_from_file_location('r8z_reference_x',ROOT/'x_reference/worker.py');_FROZEN_X=importlib.util.module_from_spec(spec);spec.loader.exec_module(_FROZEN_X)
    return _FROZEN_X

def load_plan():
    global _PLAN_CACHE
    if _PLAN_CACHE is not None:return _PLAN_CACHE
    ...
    assert len(plan['variants'])*len(plan['seeds'])==plan['execution']['task_count']
...
def run_variant(vid,seed):
    ...
    if vid=='pair06_RR':return frozen_x().run_variant(vid,seed)
...
def validate_entry(entry,plan=None):
    ...
    if config['id']in plan['reference_ids']:
        exact_reference(entry,reference_records()[(config['id'],entry['seed'])]);return
...
def summarize_matched(entries):
    ...
    for (vid,seed),entry in tasks.items():
        v=entry['config']
        if v['id']in plan['reference_ids'] or 'pair'not in v:continue
        for row in entry['rows']:
            for role in ['train','validation']:
    ...
    common={'completed_tasks':len(tasks),'expected_tasks':plan['execution']['task_count'],'diagnostic_only':True,'automatic_promotion':False,'reference_replays_passed':len(refs)-len(missing_refs),'reference_replays_pending':missing_refs,'interpretation_allowed':not missing_refs}
    ...
    for i,pair in enumerate(plan['pairs']):
        ...
        for seed in plan['seeds']:
            for year in plan['folds']:
                real=row_of(f"{plan['pair_prefix']}{i:0{plan['pair_digits']}d}_RR",seed,year);controls={a:[row_of(f"{plan['pair_prefix']}{i:0{plan['pair_digits']}d}_{a}_{p}",seed,year) for p in plan['permutation_seeds']] for a in ['RP','PR','PP']}
        ...
        results.append({'id':f"{plan['pair_prefix']}{i:0{plan['pair_digits']}d}",'features':pair,'real_mean':mean('RR'),'control_means':{a:mean(a) for a in ['RP','PR','PP']},'gain_A_given_B':mean('gain_A_given_B'),'gain_B_given_A':mean('gain_B_given_A'),'joint_gain':mean('joint_gain'),
