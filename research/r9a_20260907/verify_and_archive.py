...
EXPECTED_FROZEN='8c90bebefbda1ea7c59d2d1320ec7e70115cf5a8f782b937e5fbdf9d0e8ba8eb'
...
def main():
    ...
    assert len(expected)==len(set(expected))==3810 and len(plan['columns'])==47
    ...
    completion=json.loads((OUT/'completion.json').read_text());assert completion['status']=='completed' and completion['tasks']==3810 and completion['no_model_promotion']
    state=json.loads((OUT/'state.json').read_text());assert state['status']=='completed' and state['completed']==state['total']==3810 and state['active_workers']==0 and not state.get('error')
    ...
    assert summary['interpretation_allowed'] and summary['reference_replays_passed']==3 and not summary['reference_replays_pending'] and summary['pending_tasks']==0 and len(summary['ablations'])==1128 and len(summary['permutation_arms'])==141 and len(summary['permutation_controls'])==47
    candidates={e['task_id']:e for e in state['candidates']};assert len(candidates)==3810 and set(candidates)==set(expected)
    ...
    print('Verified 3,810 records, 3 exact references, 43 frozen files and exact complete summary.',flush=True)
