...
def research_state():
    ...
    try:
        ...
        current_stack_test=f"{HERE}/r9stacktest/results/test_result.json"
        if os.path.exists(current_stack_test):
            result=json.load(open(current_stack_test))
            full=result['aggregate']['full_legacy']
            observed=result['aggregate']['complete_target_subset']
            s['previous_test_result']=s.get('test_result')
            s.pop('test_pending',None)
            matched=sum(r['full_legacy']['score'] for r in result['rows'] if r['season']>=2020)/6
            s['test_result']={
                'protocol':result['protocol'],'score':full['score'],'observed_score':observed['score'],
                'mean_seed_score':full['mean_seed_score'],'matched_2020_2025_score':matched,
                'n_full':sum(r['n_full'] for r in result['rows']),
                'n_observed':sum(r['n_complete_target'] for r in result['rows']),
                'title':'TabICL + Ridge + CatBoost stack · 2019–2025 test',
                'description':f"Frozen equal-weight stack; TabICL16 and Ridge30 use127 inputs, CatBoost uses44. All seven classes predicted before scoring. 2020–2025 comparable-year mean: {100*matched:.2f}%. Training labels obey year−1;2024/2025 have verified labels through2022. Reused benchmark and retrospective sources; population completeness remains limited.",
                'rows':[{'season':r['season'],'k':r['k'],'n':r['n_full'],
                    'stack':r['full_legacy']['score'],'draft':r['full_legacy']['draft'],
                    'observed_n':r['n_complete_target'],'observed_score':r['complete_target_subset']['score'],
                    'label_max':r['training']['actual_admitted_label_max']} for r in result['rows']]}
            if '/r9stacktest/' in path:
                selected=json.load(open(f"{HERE}/r9stacktest/selection.json"))
                s['best_configuration']={'id':selected['selected_architecture'],'score':selected['primary_development_score'],'seeds':3,'kind':'fixed_family_stack','scope':'pre-2019 development only'}
                s['research_notice']='2019–2025 stack test complete. Selection used pre-2019 development only; all seven predictions were frozen before answers. Full-pool test correlation is the goal metric. Historic source vintage and population limitations remain.'
                s['queued']=None
                s['queued_followup']=None

        if '/r9recoveredtest/' in path:
            s['is_fixed_recovered_test']=True
            s['research_notice']='Fixed recovered stack: 2019–2025 benchmark, 182 fits. Archived weights and observed-prefix target fixed before this run; all seven predictions freeze before answers. 127 reviewed inputs differ from the old model; historical source and population limitations remain.'
            s['queued']=None
            s['queued_followup']=None
            s.pop('best_configuration',None)
            s.pop('test_pending',None)
            recovered=f"{HERE}/r9recoveredtest/results/test_result.json"
            if os.path.exists(recovered):
                result=json.load(open(recovered));mode=result['primary_mode'];recipe=result['primary_recipe']
                observed=result['aggregate']['complete_target_subset']['predictions'][mode][recipe]
                s['previous_test_result']=s.get('test_result')
                s['test_result']={'protocol':result['protocol'],'score':result['primary_score'],'observed_score':observed,
                    'matched_2020_2025_score':result['matched_2020_2025_score'],
                    'n_full':sum(r['n_full'] for r in result['rows']),'n_observed':sum(r['n_complete_target'] for r in result['rows']),
                    'title':'Recovered fixed stack · 2019–2025 test',
                    'description':'Archived blend: rich inputs25%TabICL+75%Ridge, thin inputsTabICL plus residualXGBoost; fixed three-seed family rank average. Date-limited observed prefixes,127 reviewed inputs. All seven predictions frozen before answers. Training labels end2022; this is a reused diagnostic benchmark and an adaptation, not exact reproduction of the original model.',
                    'rows':[{'season':r['season'],'k':r['k'],'n':r['n_full'],'stack':r['metrics']['full_legacy']['predictions'][mode][recipe],
                        'draft':r['metrics']['full_legacy']['draft'],'observed_n':r['n_complete_target'],
                        'observed_score':r['metrics']['complete_target_subset']['predictions'][mode][recipe],
                        'label_max':r['actual_label_max']} for r in result['rows']]}
