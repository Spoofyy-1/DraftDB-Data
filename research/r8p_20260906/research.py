...
def main():
    ...
    state.update(status='running',phase='R8p · Dated report families',total=len(tasks),completed=len(done),workers=workers,best=None,confirmation=None,test_result=None,message='Real statistics versus three matched shuffles; fixed base columns, missingness and dimensions. Test answers inaccessible.')
    write(state)
    with cf.ProcessPoolExecutor(max_workers=workers,mp_context=mp.get_context('spawn')) as pool:
        ...
        while todo or pending:
            ...
            good=[r for r in state['candidates'] if 'score' in r and r.get('config',{}).get('arm') != 'permuted']
            ...
            write(state)
    state.update(status='completed',phase='R8p · Dated report families completed',current='Queue complete',message='Matched feature controls complete. Judge real-minus-shuffled paired effects; no test-based selection.')
    write(state)

if __name__=='__main__':
    import sys
    if '--prepare-only' in sys.argv:
        from worker import _prepared
        _,_,_,folds=_prepared()
        expected=json.loads((ROOT/'baseline_reference.json').read_text())
        fields=['base_columns_hash','training_pid_hash','training_labels_hash','training_matrix_hash']
        for fold in folds:
            for key in fields:
                assert fold['audit'][key]==expected[str(fold['year'])][key], (fold['year'],key)
        (OUT/'baseline_preparation_check.json').write_text(json.dumps({'passed':True,'fields':fields,'folds':[f['year'] for f in folds]},indent=2))
        print('R8p CPU preparation exactly matches R8n baseline audit hashes.',flush=True)
    else:
        main()
