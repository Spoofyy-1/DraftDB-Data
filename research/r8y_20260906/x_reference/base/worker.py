"""R8w: all F50 singles on canonical college+dated-consensus.
...
def _load_inputs():
    ...
    sources = {}
    for family in ['combine', 'consensus', 'bio', 'fifty']:
    ...
    fifty_prov=json.loads((DATA/'fifty_provenance.json').read_text())
    fifty_metadata=json.loads((DATA/'fifty_metadata.json').read_text())
    lineage=json.loads((DATA/'fifty_lineage.json').read_text())
    assert len(plan['families']['fifty'])==50 and all(c.startswith('f50_') for c in plan['families']['fifty'])
    assert '45' not in fifty_metadata['source_fields']
    assert all(int(filename.split('_')[1].split('.')[0])<=2018 for filename in fifty_metadata['source_hashes'])
    known={r['pid']:r for r in fifty_prov};traced={r['pid']:r for r in lineage}
    assert len(known)==len(fifty_prov) and len(traced)==len(lineage)
    observed=set(sources['fifty'].loc[sources['fifty'][plan['families']['fifty']].notna().any(axis=1),'pid'])
    assert observed<=set(known)<=set(traced)
    for pid,row in known.items():
        assert pid in year_by_pid and row['draft_year']==year_by_pid[pid]<=2018
        assert row['source_season']<=row['draft_year'] and max(row['history_seasons'])==row['source_season']
        trace=traced[pid]
        assert trace['draft_year']==row['draft_year'] and trace['focal']['season']==row['source_season']
        assert sorted(r['season'] for r in trace['history'])==sorted(row['history_seasons'])
        assert not trace['team_has_duplicate_tpid']
        for source_row in [trace['focal']]+trace['history']+trace['team']:
            assert source_row['season']<=row['draft_year'] and len(source_row['allowed_values_hash'])==64
...
def _vector_shuffle(values, metadata, family, seed, role):
    assert role in ['train', 'validation'] and (family == 'consensus' or family.startswith('fifty:'))
...
def _design(fold, variant, plan):
    ...
    assert variant['feature'] is None if variant['extra_arm'] == 'absent' else variant['feature'] in plan['single_features']
    family_audits, ordered_slots = {}, []
    for family in ['consensus', 'fifty']:
        ...
        stream = family if family == 'consensus' else 'fifty:' + variant['feature']
        for role, frame, target in [('train', fold['tr'], atr), ('validation', fold['te'], ate)]:
            ...
            for feature in features:
                target[plan['slot_mapping'][feature] if family == 'consensus' else plan['common_extra_slot']] = values[feature]
            family_audits[family][role] = audit
        ordered_slots.extend(plan['slot_mapping'][feature] if family == 'consensus' else plan['common_extra_slot'] for feature in features)
    assert list(atr) == list(ate) == fold['audit']['ordered_base_columns'] + ordered_slots
    assert len(atr.columns) == (45 if variant['extra_arm'] == 'absent' else 46)
...
def summarize_matched(candidates):
    ...
    for (vid, _), entry in tasks.items():
        ...
        for row in entry['rows']:
            ...
            expected_slots = [plan['slot_mapping'][column] for column in plan['families']['consensus']]
            if variant['extra_arm'] != 'absent': expected_slots.append(plan['common_extra_slot'])
            ...
            for family, roles in row['audit']['families'].items():
                ...
                for role, audit in roles.items():
                    assert audit['features'] == (plan['families']['consensus'] if family == 'consensus' else [variant['feature']])
                    streams[(family, variant['feature'] if family == 'fifty' else None, mode, row['season'], role)].add(audit['matrix_hash'])
    ...
    for ref in reference['references']:
        key=('baseline',ref['seed'])
        if key not in tasks:replay_pending.append(ref['seed']);continue
        current=tasks[key]
        assert current['score']==ref['score'],('Baseline score mismatch',key)
        assert all(a['season']==b['season'] and a['stack']==b['stack'] and a['predictions']==b['predictions'] for a,b in zip(current['rows'],ref['rows'])),('Baseline prediction mismatch',key)
        replay.append({'seed':ref['seed'],'score_and_raw_canonical_predictions_exact':True})
    if replay_pending:
        return {'statistics':[],'pending':plan['single_features'],'completed_tasks':len(tasks),'expected_tasks':603,
            'baseline_replay_pending':replay_pending,'baseline_replay_checks':replay,'interpretation_allowed':False,
            'baseline_score_context_only':baseline,'diagnostic_only':True,'automatic_promotion':False}
    for feature in plan['single_features']:
        vids = [v['id'] for v in plan['variants'] if v['feature'] == feature]
        ...
        for seed in plan['seeds']:
            for year in plan['folds']:
                real = row(feature + '_real', seed, year)
                controls = [row(f'{feature}_shuffle{pseed}', seed, year) for pseed in plan['permutation_seeds']]
                for control in controls: _check_matched(real, control)
        ...
        statistics.append({'id':feature,'paired_mean_gain':float(np.mean([p['gain'] for p in paired])),
            'real_mean':float(np.mean([p['real'] for p in paired])),'control_mean':float(np.mean([p['mean_control'] for p in paired])),
            'real_minus_baseline_context_only':float(np.mean([p['real'] for p in paired]))-baseline,
            'paired_rows':paired,'fold_gains':{str(y):float(np.mean([p['gain'] for p in paired if p['season']==y])) for y in plan['folds']},
            'seed_gains':{str(seed):float(np.mean([p['gain'] for p in paired if p['seed']==seed])) for seed in plan['seeds']}})
    return {'statistics':statistics,'pending':pending,'completed_tasks':len(tasks),'expected_tasks':603,
        'baseline_replay_pending':[],'baseline_replay_checks':replay,'interpretation_allowed':True,
        'baseline_score_context_only':baseline,'diagnostic_only':True,'automatic_promotion':False,
        'ordering_policy':plan['ordering_policy'],'earlier_raw_study_scores_pooled':False,
        'interpretation':'Each F50 field is paired with same-mask/cohort controls on fixed45-column inputs plus one common slot. Fifty exploratory comparisons are not significance tests. Positive matched gain alone does not establish baseline improvement. No automatic promotion, combinations, confirmation or held-out scoring.'}
