...
def main():
    ...
    state.update(status='running',phase='R8r · Source college 50-statistic controls',total=len(tasks),completed=len(done),workers=workers,best=None,confirmation=None,test_result=None,message='Real statistics versus three matched shuffles; fixed base columns, missingness and dimensions. Test answers inaccessible.')
    write(state)
    with cf.ProcessPoolExecutor(max_workers=workers,mp_context=mp.get_context('spawn')) as pool:
        ...
        while todo or pending:
            ...
            write(state)
    state.update(status='completed',phase='R8r · Source college 50-statistic controls completed',current='Queue complete',message='Matched feature controls complete. Judge real-minus-shuffled paired effects; no test-based selection.')
    write(state)
...
if __name__=='__main__':
    ...
    if '--prepare-only' in sys.argv:
        ...
        for backbone in expected:
            _,_,_,folds=_prepared(backbone)
            for fold in folds:
                for key in fields:
        ...
        print('R8r CPU preparation exactly matches R8n source-only baseline audit hashes.',flush=True)
    else:
        main()
