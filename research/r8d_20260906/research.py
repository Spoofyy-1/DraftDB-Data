...
def main():
    ...
    state.update(status='running',phase='R8d · parallel combinations',total=len(tasks),completed=len(done),workers=workers,best=None,confirmation=None,test_result=None,message='Fixed development combinations; test answers are inaccessible. Three independent seeds per configuration.')
    write(state)
    with cf.ProcessPoolExecutor(max_workers=workers,mp_context=mp.get_context('spawn')) as pool:
        ...
        while todo or pending:
            ...
            write(state)
    state.update(status='completed',phase='R8d · combinations completed',current='Queue complete',message='Compare configuration means across seeds, not the luckiest individual seed. No test-based selection.')
    write(state)
