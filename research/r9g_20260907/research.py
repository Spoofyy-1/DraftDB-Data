"""Frozen R9g study using the unchanged benchmark-verified scheduler."""
import os
...
def main():
    frozen=verify_frozen();plan=X.load_plan();gpu=json.loads((OUT/'gpu_fixture.json').read_text());assert gpu['passed']and gpu['models_scored']==0 and len(gpu['fixtures'])==2 and gpu['frozen_manifest_sha256']==hashlib.sha256((ROOT/'frozen.json').read_bytes()).hexdigest();check=json.loads((OUT/'preparation.json').read_text());assert check['passed'] and check['frozen_manifest_sha256']==hashlib.sha256((ROOT/'frozen.json').read_bytes()).hexdigest()
    ...
    def event(kind,detail):
        if kind=='snapshot_end':
            path=OUT/'state.json';state=json.loads(path.read_text());state.update(phase='R9g XGBoost GPU fixed checkpoints',started=started,updated=time.time(),confirmation=None,test_result=None)
    ...
    state=json.loads((OUT/'state.json').read_text());state.update(status='postprocessing',phase='R9g registered saved-prediction blends',active_workers=0,current='All model fits complete; computing fixed saved-prediction blends.')
    Q.atomic_json(OUT/'state.json',state)
    import postprocess
    blends=postprocess.process(result['records'],OUT)
    state=json.loads((OUT/'state.json').read_text());state.update(status='completed',phase='R9g complete',current='',saved_blends=blends);Q.atomic_json(OUT/'state.json',state)
...
if __name__=='__main__':
    try:
        if '--cpu-fixture'in sys.argv:import preflight;preflight.prepare(gpu_fixture=False)
        elif '--gpu-fixture'in sys.argv:verify_frozen();import preflight;preflight.prepare(gpu_fixture=True)
