...
def main():
    ...
    assert plan['task_count'] == len(plan['tasks']) == 24
    reuse_records,reuse_gate=reuse_controls.verify(ROOT)
    assert len(reuse_records)==6
    results = []
    def one(task):
    ...
    tests = []
    for relative in ['code/test_worker.py','test_cross_panel.py']:
    ...
    summary = {'passed': True, 'models_fitted': 0, 'tasks_checked': len(results), 'tasks': results,
               'tests': tests, 'reuse_gate': reuse_gate, 'runtime_sources_checked': runtime_paths,
