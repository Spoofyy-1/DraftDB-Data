...
def prepare():
    ...
    for fold in folds:
        ...
        for seed in p['seeds']:
            old=next(r for r in X.references()[seed]['rows'] if r['season']==fold['year'])
    ...
    Q.atomic_json(OUT/'preparation.json',{'passed':True,'models_fitted':0,'source_designs':proof,'requested_configurations':60,'distinct_configurations':len(p['variants']),'tasks':p['execution']['task_count'],'disabled_serialization_safe':True,'frozen_manifest_sha256':hashlib.sha256((ROOT/'frozen.json').read_bytes()).hexdigest()})
    print('R9d source/reference/constructor/CPU-payload integrity preflight passed; no model fits.',flush=True)
...
def main():
    ...
    def event(kind,detail):
        if kind=='snapshot_end':
            path=OUT/'state.json';state=json.loads(path.read_text());state.update(phase='R9d registered TabICL settings',started=started,updated=time.time(),confirmation=None,test_result=None)
