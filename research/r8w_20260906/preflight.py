...
assert len(plan['variants'])==201 and plan['execution']['task_count']==603
...
for year in plan['folds']:
    ...
    assert sources==plan['training_eligibility_registration'][str(year)] and len(sources['fifty'])==50
    for feature in plan['single_features']:
        ...
        for role,frame in [('train',tr),('validation',te)]:
            ...
            for seed in plan['permutation_seeds']:
                shuffled=W._vector_shuffle(values,frame,'fifty:'+feature,seed,role)
                rev=frame.iloc[::-1];other=W._vector_shuffle(rev[[feature]],rev,'fifty:'+feature,seed,role)
    ...
    for variant in plan['variants']:
        ...
        if variant['feature']:assert list(atr)[-1]==plan['common_extra_slot'] and families['fifty']['train']['features']==[variant['feature']]
...
for variant in plan['variants']:
    for seed in plan['seeds']:
        ...
        for fold in folds:
            ...
            if variant['extra_arm']=='real':score+=(plan['single_features'].index(variant['feature'])-5)*.01
            ...
            if variant['id']=='baseline':
                ref=next(r for r in reference['references'] if r['seed']==seed)
                rr=next(r for r in ref['rows'] if r['season']==fold['year'])
                score=rr['stack'];design['predictions']=copy.deepcopy(rr['predictions'])
                design['audit']['canonical_prediction_ties']=copy.deepcopy(rr['audit']['canonical_prediction_ties'])
...
assert summary['completed_tasks']==603 and not summary['pending'] and summary['interpretation_allowed']
assert len(summary['baseline_replay_checks'])==3
assert not W.summarize_matched([e for e in fixtures if e['config']['id']!='baseline'])['interpretation_allowed']
...
rejects('wrong_single_field',lambda r:r['rows'][0]['audit']['families']['fifty']['train'].__setitem__('features',[plan['single_features'][1]]))
...
rejects('changed_mask',lambda r:r['rows'][0]['audit']['families']['fifty']['train']['invariants'].__setitem__('mask_hash','bad'))
...
report={'passed':True,'gpu_models_run':0,'cpu_selector_run':False,'task_count':603,'source_file_hashes_checked':len(reference['source_data_hashes']),
        ...
        'server_preflight_required':'Both W and unchanged copied U CPU preparation must exactly match completed U baseline: full45-column matrices, labels, pids and columns. All3 baseline prediction/score replays must pass before interpretation.'}
