"""R8u: individual dated biography screen on canonical college+consensus.
...
def summarize_matched(candidates):
    ...
    for (vid, _), entry in tasks.items():
        ...
        for row in entry['rows']:
            ...
            if variant['bio_arm'] != 'absent': expected_slots.append(plan['common_bio_slot'])
            ...
            for family, roles in row['audit']['families'].items():
                ...
                for role, audit in roles.items():
                    ...
                    streams[(family, variant['feature'] if family == 'bio' else None, mode, row['season'], role)].add(audit['matrix_hash'])
    ...
    return {'statistics':statistics,'pending':pending,'completed_tasks':len(tasks),'expected_tasks':123,
        ...
        'interpretation':'Each single field is paired with same-mask/cohort controls on identical45-column fixed inputs plus one common added slot. Baseline width differs. Ten exploratory comparisons after a harmful family result do not establish significance; no automatic promotion, combinations, confirmation or held-out scoring.'}
