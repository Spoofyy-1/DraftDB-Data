...
def main():
    ...
    state.update(status='running',phase='R8w · F50 with dated consensus',total=len(tasks),completed=len(done),workers=workers,best=None,confirmation=None,test_result=None,message='Real statistics versus three matched shuffles; fixed base columns, missingness and dimensions. Test answers inaccessible.')
    write(state)
    with cf.ProcessPoolExecutor(max_workers=workers,mp_context=mp.get_context('spawn')) as pool:
        ...
        while todo or pending:
            ...
            write(state)
    state.update(status='completed',phase='R8w · F50 with dated consensus completed',current='Queue complete',message='Matched feature controls complete. Judge real-minus-shuffled paired effects; no test-based selection.')
    write(state)
...
if __name__=='__main__':
    ...
    if '--prepare-only' in sys.argv:
        ...
        for fold,ufold in zip(folds,ufolds):
            year=fold['year'];expected=next(r['audit'] for r in reference['references'][0]['rows'] if r['season']==year)
            uvariant=next(v for v in uplan['variants'] if v['id']=='baseline')
            variant=next(v for v in plan['variants'] if v['id']=='baseline')
            atr,ate,families=_design(fold,variant,plan);utr,ute,ufamilies=U._design(ufold,uvariant,uplan)
            ...
            for key in ['ordered_base_columns','base_columns_hash','training_pid_hash','validation_pid_hash','training_labels_hash','training_matrix_hash','validation_matrix_hash','ordering_policy_hash','input_columns','raw_feature_count','training_nonconstant_columns']:
                assert merged[key]==expected[key],(year,key)
            ...
            assert len(atr.columns)==45
            audits.append({'year':year,'all_completed_R8u_hashes_match':True,'full_training_matrix_hash':_matrix_hash(atr),
        ...
        check={'passed':True,'exact_completed_R8u_baseline_match':True,'folds':audits,'constructor_checks':configurations,
        ...
        print('R8w completed R8u baseline/component/full-matrix/label/identity/column hashes verified; no GPU prediction.',flush=True)
    else:
        check=json.loads((OUT/'baseline_preparation_check.json').read_text())
        assert check['passed'] and check['exact_completed_R8u_baseline_match']
