...
from worker import run_variant, summarize_pairs

ROOT=Path(__file__).resolve().parent
OUT=ROOT/'results'
def write(state):
    state['updated']=time.time()
    state['matched_summary']=summarize_pairs(state.get('candidates',[]))
    tmp=OUT/'state.tmp'
...
def main():
    ...
    state.update(status='running',phase='R8k · Matched complementary pairs',total=len(tasks),completed=len(done),workers=workers,best=None,confirmation=None,test_result=None,message='Real statistics versus three matched shuffles; fixed base columns, missingness and dimensions. Test answers inaccessible.')
    write(state)
    with cf.ProcessPoolExecutor(max_workers=workers,mp_context=mp.get_context('spawn')) as pool:
        ...
        while todo or pending:
            ...
            write(state)
    state.update(status='completed',phase='R8k · Matched complementary pairs completed',current='Queue complete',message='Matched complementary pairs complete. Judge real-minus-shuffled paired effects; no test-based selection.')
    write(state)
