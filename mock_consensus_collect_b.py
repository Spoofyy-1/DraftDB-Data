...
for y,d in DRAFT.items():
    ...
    for site,pat in SITES.items():
        u=pat.format(y=y); cdx=fetch(f"https://web.archive.org/cdx/search/cdx?url={urllib.parse.quote(u)}&from={frm}&to={to}&output=json&filter=statuscode:200&fl=timestamp,original&limit=-1",f"mock_raw/consensus2/cdx_{site}_{y}.json")
...
w=csv.DictWriter(open("mock_consensus2_ranks_b.csv","w"),fieldnames=["pid","draft_year","site","rank","ts"]); w.writeheader(); [w.writerow(r) for r in rows]
