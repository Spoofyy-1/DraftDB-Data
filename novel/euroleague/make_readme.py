#!/usr/bin/env python3
"""Regenerate README.md with coverage numbers computed from the current
outputs, so the documentation can never drift from the data."""
import os
import subprocess
...
def main():
    ...
    unm = pd.read_csv(os.path.join(HERE, "unmatched.csv")) if os.path.exists(
        os.path.join(HERE, "unmatched.csv")) else pd.DataFrame(columns=["reason"])
    matches = pd.read_csv(os.path.join(HERE, "matches.csv"))
    ident = pd.read_csv(IDENT)

    # ---- pull coverage per competition ----
    comp_name = {"E": "Euroleague", "U": "EuroCup", "J": "U18 ANGT"}
    rows = []
    for c, g in cov.groupby("comp"):
        rows.append({
            "code": c, "competition": comp_name.get(c, c),
            "seasons": len(g),
            "first": g.first_game.replace("", pd.NA).dropna().min(),
            "last": g.last_game.replace("", pd.NA).dropna().max(),
            "games played": int(g.games_played.sum()),
            "box scores cached": int(g.box_scores_cached.sum()),
            "% cached": f"{100*g.box_scores_cached.sum()/max(g.games_played.sum(),1):.1f}%",
            "seasons complete": f"{int(g.complete.sum())}/{len(g)}",
        })
    pull_tbl = md_table(pd.DataFrame(rows))
    total_played = int(cov.games_played.sum())
    total_cached = int(cov.box_scores_cached.sum())
    all_done = int(cov.complete.sum()) == len(cov)

    # ---- coverage by draft-year band ----
    dy = dict(zip(ident.pid, ident.draft_year))
    prov["draft_year"] = prov.pid.map(dy)
    feat["draft_year"] = feat.pid.map(dy)
    bands = [("2000-07", 2000, 2007), ("2008-18", 2008, 2018), ("2019-25", 2019, 2025),
             ("2026", 2026, 2026)]
    rows = []
    for name, lo, hi in bands:
        pool = ident[(ident.draft_year >= lo) & (ident.draft_year <= hi)]
        got = feat[(feat.draft_year >= lo) & (feat.draft_year <= hi)]
        sub = got
        rows.append({
            "draft years": name,
            "drafted players in identity file": len(pool),
            "with pre-draft intl rows": len(got),
            "% of class": f"{100*len(got)/max(len(pool),1):.1f}%",
            "with Euroleague mins": int((sub.el2_min > 0).sum()) if len(sub) else 0,
            "with EuroCup mins": int((sub.eu2_min > 0).sum()) if len(sub) else 0,
            "with ANGT": int(sub.angt_has.fillna(0).sum()) if len(sub) else 0,
        })
    band_tbl = md_table(pd.DataFrame(rows))

    # ---- unmatched reasons ----
    if len(unm):
        ur = unm.reason.value_counts().rename_axis("reason").reset_index(name="persons")
        unm_tbl = md_table(ur)
    else:
        unm_tbl = "_(none)_"

    # ---- fill rates ----
    blocks = {"el2_": "Euroleague", "eu2_": "EuroCup", "yng_": "young senior minutes",
              "angt_": "U18 ANGT", "club_": "clubs / registrations",
              "tier_": "league tier", "bio_": "bio"}
    rows = []
    for pre, lab in blocks.items():
        cols = [c for c in feat.columns if c.startswith(pre)]
        if not cols:
            continue
        rows.append({"block": pre + "*", "meaning": lab, "columns": len(cols),
                     "median fill rate": f"{100*feat[cols].notna().mean().median():.0f}%"})
    fill_tbl = md_table(pd.DataFrame(rows))

    n_cols = len([c for c in feat.columns if c not in ("pid", "draft_year")])
    try:
        pgq = pd.read_csv(os.path.join(HERE, "player_games.csv.gz"),
                          usecols=["person_code"], low_memory=False)
        n_persons = pgq.person_code.nunique()
    except Exception:
        n_persons = 0
    status = ("**COMPLETE** - every competition-season has been pulled."
              if all_done else
              f"**PARTIAL** - {total_cached:,}/{total_played:,} box scores cached "
              f"({100*total_cached/max(total_played,1):.1f}%). "
              f"See _Resuming_ below; rerun `build.py` after the pull finishes.")

    txt = f"""# euroleague - international data spine (Euroleague / EuroCup / U18 ANGT)
...
Generated {datetime.now().strftime('%Y-%m-%d %H:%M')}.  Pull status: {status}
...
`{os.path.basename(IDENT)}` (2,560 drafted players, 2000-2025) plus verified
...
Result: **{len(matches)} person-code matches covering {matches.pid.nunique()} pids**
({int(matches.birth_day_mismatch.sum()) if "birth_day_mismatch" in matches.columns else 0} of them
matched on birth year with a flagged day-level mismatch),
of which **{len(feat)} have at least one pre-draft game** and appear in
`features.csv`.  `unmatched.csv` holds {len(unm)} rows:
...
| file | contents |
|---|---|
| `features.csv` | **the deliverable** - `pid` + {n_cols} numeric columns, one row per pid |
...
"""
    with open(os.path.join(HERE, "README.md"), "w") as fh:
        fh.write(txt)
    print(f"wrote README.md ({len(txt)} chars); pull status: "
          f"{'complete' if all_done else 'partial'}")
